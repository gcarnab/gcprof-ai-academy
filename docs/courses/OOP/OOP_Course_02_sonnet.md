...
```
        STAMPO (Classe)                  BISCOTTI (Oggetti)
      ┌────────────────┐
      │   forma:  ★     │        ──▶   🍪 biscotto 1 (gusto: cioccolato)
      │ ingredienti: ?  │        ──▶   🍪 biscotto 2 (gusto: vaniglia)
      └────────────────┘         ──▶   🍪 biscotto 3 (gusto: limone)

  Un solo stampo (UNA classe)  →  infiniti biscotti (MOLTI oggetti),
  tutti con la stessa FORMA ma ciascuno con il PROPRIO gusto.
```
...
```
class Studente:      →      studente1 = Studente()   →   studente2 = Studente()
    (progetto)                    (oggetto 1)                  (oggetto 2)
                              nome: "Mario Rossi"          nome: "Giulia Bianchi"
                              eta:  16                     eta:  17
...
```
   studente1 ──▶ [ nome: "Mario Rossi" | eta: 18 ]     ← area di memoria 1
   studente2 ──▶ [ nome: "Giulia Bianchi" | eta: 17 ]  ← area di memoria 2 (SEPARATA)

        Modificare l'area 1 non tocca in alcun modo l'area 2.
```
