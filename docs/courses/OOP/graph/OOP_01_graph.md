...
In questo modulo non scriverai ancora una singola riga di codice a oggetti (arriverà nel Modulo 2), se non una breve anteprima da leggere e non da scrivere: l'obiettivo è cambiare **il modo in cui guardi un problema**, prima di cambiare il modo in cui lo scrivi. È il passaggio più importante di tutto il corso: chi lo salta, impara solo la sintassi dell'OOP senza mai capire perché esiste.
...
```mermaid
flowchart LR
    A["📋 Caratteristiche<br/>i dati che lo descrivono"]:::ca --> O(["🧱 OGGETTO"]):::co
    B["⚙️ Comportamenti<br/>le azioni che sa compiere"]:::cb --> O
    classDef ca fill:#d6e6fb,stroke:#264de4,color:#222
    classDef cb fill:#fde8d0,stroke:#e34c26,color:#222
    classDef co fill:#e3f4dc,stroke:#3d8b37,color:#222
```

*📊 Un oggetto = caratteristiche + comportamenti, raggruppati insieme.*

Nel linguaggio tecnico le caratteristiche si chiamano **attributi** e i comportamenti si chiamano **metodi**: sono i due termini che userai da qui in avanti.
...
Ecco tre degli esempi precedenti visti come "schede" di oggetti:
...
*📊 Ogni scheda riporta il nome dell'oggetto, le sue caratteristiche (sopra) e i suoi comportamenti (sotto, con le parentesi).*
...
Basta un dizionario scritto male — ad esempio `{"nome": "Luca"}`, senza la chiave `"voti"` — perché `calcola_media()` interrompa il programma con un `KeyError`. È questo il tipo di fragilità che l'OOP aiuta a ridurre.
...
```mermaid
flowchart TD
    subgraph PROC["🧾 Stile procedurale"]
        D["studente1, studente2<br/>dati sparsi"]:::data -- "passati a" --> F["calcola_media()<br/>funzione esterna"]:::fn
    end
    subgraph OOP["🧱 Oggetto Studente"]
        A["nome, voti<br/>attributi"]:::data ---|"insieme"| M["calcola_media()<br/>metodo"]:::fn
    end
    classDef data fill:#d6e6fb,stroke:#264de4,color:#222
    classDef fn fill:#fde8d0,stroke:#e34c26,color:#222
```

*📊 Stessa logica, due organizzazioni: a sinistra dati e funzione vanno collegati a mano; a destra il comportamento vive dentro l'oggetto.*
...
> 💡 **Da ricordare** — I vantaggi dell'OOP riguardano la *qualità dell'organizzazione* del codice, non la velocità di esecuzione: un programma a oggetti non è automaticamente più veloce di uno procedurale.
...
flowchart LR
    O["🧱 OOP"]:::root
    O --> A["🔒 Incapsulamento<br/>Modulo 4"]:::p
    O --> B["🧬 Ereditarietà<br/>Modulo 5"]:::p
    O --> C["🎭 Polimorfismo<br/>Modulo 6"]:::p
    O --> D["🧩 Astrazione<br/>Modulo 7"]:::p
    classDef root fill:#e3f4dc,stroke:#3d8b37,color:#222
    classDef p fill:#fff6c2,stroke:#c9a800,color:#222
```

*📊 I quattro pilastri dell'OOP e il modulo in cui li approfondirai.*
...
```mermaid
flowchart LR
    R["🧱 Oggetto<br/>dati + azioni"]:::root
    R --> B["🏦 Banca<br/>Conto: saldo, prelievo"]:::leaf
    R --> M["📣 Marketing<br/>Cliente: ordini, valore"]:::leaf
    R --> V["🎮 Videogiochi<br/>Personaggio: vita, attacco"]:::leaf
    R --> I["🌍 Rel. internazionali<br/>Delegazione: obiettivi"]:::leaf
    classDef root fill:#e3f4dc,stroke:#3d8b37,color:#222
    classDef leaf fill:#d6e6fb,stroke:#264de4,color:#222
```

*📊 Domini diversi, stesso schema: ovunque ci sia un'entità, c'è un oggetto con dati e azioni.*
...
Ecco lo schema da usare come traccia per i punti 2 e 3:
...
*📊 Traccia della scheda: almeno 4 caratteristiche e almeno 3 azioni.*
...
- Se hai un dubbio su come classificare qualcosa, usa questa regola pratica: se è qualcosa che l'oggetto **ha**, è un attributo; se è qualcosa che l'oggetto **fa**, è un metodo.
...
- ❌ *"Ogni sostantivo del problema deve diventare un oggetto."* → Diventano oggetti solo le entità che hanno dati e comportamenti rilevanti per il problema; il resto sono semplici attributi (il "colore" di un'automobile è un attributo, non un oggetto da progettare).
...
| Attributo e metodo | Ciò che l'oggetto **ha** (un dato) e ciò che l'oggetto **fa** (un'azione) |
