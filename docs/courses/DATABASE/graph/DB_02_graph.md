...
```mermaid
flowchart LR
    U["👤 Tu"] -->|autorizzi| A["🔐 google.colab.auth"] --> G["📊 Google Sheets / Drive"]
    style U fill:#dbeafe,stroke:#1e3a8a
...
```mermaid
flowchart LR
    C["🆕 Create<br/>append_row()"] --> R["👁️ Read<br/>get_all_records()"] --> U["✏️ Update<br/>update_cell()"] --> D["🗑️ Delete<br/>delete_rows()"]
    style C fill:#d1fae5,stroke:#065f46
    style R fill:#dbeafe,stroke:#1e3a8a
    style U fill:#fef3c7,stroke:#92400e
...
```mermaid
flowchart LR
    S["📄 Google Sheet"] -->|"gspread"| L["Lista di<br/>dizionari"] -->|"pd.DataFrame()"| DF["🐼 DataFrame<br/>Pandas"]
    style S fill:#dbeafe,stroke:#1e3a8a
    style L fill:#fef3c7,stroke:#92400e
    style DF fill:#d1fae5,stroke:#065f46
```
...
```mermaid
flowchart LR
    A["⚙️ Setup<br/>e Auth"] --> B["🆕 Create<br/>(append_row)"] --> C["👁️ Read<br/>(get_all_records)"] --> D["✏️🗑️ Update/Delete<br/>(find, update_cell, delete_rows)"] --> E["🐼 DataFrame<br/>Pandas"]
    style A fill:#fef3c7,stroke:#92400e
    style B fill:#d1fae5,stroke:#065f46
    style C fill:#dbeafe,stroke:#1e3a8a
    style D fill:#fed7aa,stroke:#c2410c
    style E fill:#e0e7ff,stroke:#3730a3
```
