...
```mermaid
flowchart LR
    A["🗣️ Tu scrivi<br/>'stampa Ciao'"] --> B["🐍 Interprete Python<br/>traduce riga per riga"]
    B --> C["💻 Computer (binario)<br/>esegue subito"]
    style A fill:#dbeafe,stroke:#1e3a8a
    style B fill:#fef3c7,stroke:#92400e
    style C fill:#d1fae5,stroke:#065f46
```

*Non serve preparare tutta la traduzione prima: ogni riga viene tradotta ed eseguita "al volo", mentre viene letta.*
...
```mermaid
flowchart TD
    P["🐍 Python<br/>multi-paradigma"] --> PR["Procedurale<br/>(istruzioni in sequenza)"]
    P --> OOP["Orientato agli oggetti<br/>(classi e oggetti)"]
    P --> FN["Funzionale<br/>(funzioni come valori)"]
    style P fill:#e0e7ff,stroke:#3730a3,stroke-width:1.5px
    style PR fill:#dbeafe,stroke:#1e3a8a
    style OOP fill:#fef3c7,stroke:#92400e
    style FN fill:#d1fae5,stroke:#065f46
```
...
```mermaid
flowchart TD
    T1["📄 Cella di TESTO<br/>spiegazione, titoli (non eseguita)"] --> C1["🐍 Cella di CODICE<br/>istruzioni Python (Shift+Invio)"]
    C1 --> T2["📄 Cella di TESTO<br/>altra spiegazione"]
    T2 --> C2["🐍 Cella di CODICE<br/>può usare variabili create sopra"]
    style T1 fill:#f3f4f6,stroke:#374151
    style T2 fill:#f3f4f6,stroke:#374151
    style C1 fill:#dbeafe,stroke:#1e3a8a
    style C2 fill:#dbeafe,stroke:#1e3a8a
```
...
### 🔍 Come leggere un messaggio di errore

```mermaid
flowchart TD
    S1["1️⃣ Parti dall'ultima riga:<br/>tipo di errore + descrizione"] --> S2["2️⃣ Risali al Traceback:<br/>in quale riga di codice è successo"]
    S2 --> S3["3️⃣ Individua il punto esatto<br/>(indicato da ^, se presente)"]
