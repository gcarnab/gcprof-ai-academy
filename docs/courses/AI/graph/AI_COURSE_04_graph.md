...
```mermaid
flowchart LR
    A[Python] --> B[Ambiente isolato]
    B --> C[Dipendenze]
    C --> D[Notebook e file .py]
    D --> E[NumPy · Pandas · Matplotlib]
    E --> F[OOP]
    F --> G[Debug]
    G --> H[Laboratorio]
    H --> I[Project Work]
...
**Python** è uno dei linguaggi più utilizzati nell'ecosistema dell'intelligenza artificiale: dalla ricerca alle pipeline di produzione. Non è un caso — Python ha una sintassi leggibile, un ecosistema maturo di librerie scientifiche e di machine learning (NumPy, Pandas, scikit-learn, PyTorch, TensorFlow) e una comunità molto ampia che rende disponibili strumenti e documentazione.

In questo modulo non ti limiterai a "imparare la sintassi": costruirai un **ambiente di sviluppo professionale** su VS Code, esattamente come farebbe un AI Engineer in azienda, e getterai le basi della **Programmazione Orientata agli Oggetti (OOP)**, un paradigma molto importante per comprendere e utilizzare librerie AI moderne. Molti componenti di librerie come scikit-learn e framework per reti neurali sono infatti esposti attraverso oggetti con attributi e metodi.
...
- ✅ Usare **pip** per installare e aggiornare le dipendenze e un file `requirements.txt` per documentare le dipendenze di un progetto e facilitarne la riproducibilità.
...
> 💡 **Idea chiave:** un progetto Python ben configurato separa **codice, interprete, dipendenze e configurazione**.

```mermaid
flowchart TD
    A[Progetto Python] --> B[Python / interprete]
    A --> C[Ambiente virtuale venv]
    C --> D[pip]
    D --> E[Dipendenze]
    E --> F[requirements.txt]
    A --> G[VS Code]
    G --> H[Debug · Git · Notebook]
```


**Perché lavorare in locale e non solo su un notebook cloud?**
Molti corsi introduttivi all'AI usano notebook online (Colab, Kaggle) perché sono comodi per iniziare. In un progetto reale puoi invece lavorare in locale, su server aziendali o su ambienti cloud, usando un editor completo, debugging, controllo di versione e gestione delle dipendenze. Imparare fin da subito queste pratiche aiuta a passare dall'esplorazione alla costruzione di progetti riproducibili.
...
- **Ambiente virtuale (`venv`)**: uno "spazio isolato" dove installare le librerie di un progetto, separandole dall'ambiente globale e riducendo i conflitti tra progetti diversi. `venv` è il modulo incluso nella distribuzione standard di Python; `virtualenv` è anche il nome di uno strumento esterno con funzionalità correlate.
...
> 🔎 **Controllo rapido dell'interprete:** se hai dubbi su quale Python stai usando, puoi verificare l'interprete selezionato in VS Code e controllare dal terminale il percorso dell'eseguibile. Questo evita di installare pacchetti nell'ambiente sbagliato.
...
Il file `requirements.txt` documenta le dipendenze del progetto e facilita la **riproducibilità**: chiunque può ricreare un ambiente compatibile con un solo comando. Per una riproducibilità rigorosa, però, contano anche versione di Python, sistema operativo, dipendenze transitive e, quando necessario, versioni fissate e lockfile/strumenti dedicati.

### 4.2 — Notebook in VS Code

```mermaid
flowchart LR
    A[Notebook .ipynb] --> B[Cella Markdown]
    A --> C[Cella di codice]
    C --> D[Kernel Python]
    D --> E[Output]
    B --> F[Documentazione]
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 250" role="img" aria-label="Flusso NumPy Pandas Matplotlib">
  <rect x="20" y="70" width="240" height="110" rx="18" fill="none" stroke="currentColor" stroke-width="3"/>
  <rect x="330" y="70" width="240" height="110" rx="18" fill="none" stroke="currentColor" stroke-width="3"/>
  <rect x="640" y="70" width="240" height="110" rx="18" fill="none" stroke="currentColor" stroke-width="3"/>
  <text x="140" y="112" text-anchor="middle" font-size="30" font-family="sans-serif" font-weight="bold">NumPy</text>
  <text x="140" y="145" text-anchor="middle" font-size="19" font-family="sans-serif">array · calcolo</text>
  <text x="450" y="112" text-anchor="middle" font-size="30" font-family="sans-serif" font-weight="bold">Pandas</text>
  <text x="450" y="145" text-anchor="middle" font-size="19" font-family="sans-serif">DataFrame · dati</text>
  <text x="760" y="112" text-anchor="middle" font-size="30" font-family="sans-serif" font-weight="bold">Matplotlib</text>
  <text x="760" y="145" text-anchor="middle" font-size="19" font-family="sans-serif">grafici · insight</text>
  <path d="M260 125 H330 M570 125 H640" stroke="currentColor" stroke-width="3" fill="none"/>
  <path d="M315 115 L330 125 L315 135 M625 115 L640 125 L625 135" stroke="currentColor" stroke-width="3" fill="none"/>
  <text x="450" y="35" text-anchor="middle" font-size="22" font-family="sans-serif">Dal dato numerico alla visualizzazione</text>
</svg>
...
Queste tre librerie sono fondamentali nell'ecosistema Python per dati e machine learning: le ritroverai in molti moduli successivi, insieme ad altre librerie specializzate.
...
classDiagram
    ...
    class StudenteConBonus {
        +bonus
        +media()
    }
    Studente <|-- StudenteConBonus
```


L'OOP organizza il codice attorno a **oggetti**: entità che raggruppano **dati** (attributi) e **comportamenti** (metodi). Molte librerie AI espongono modelli e componenti come oggetti; comprendere classi, istanze, metodi ed ereditarietà ti permette quindi di leggere e utilizzare meglio le loro API.
...
```mermaid
flowchart LR
    A[Errore / comportamento inatteso] --> B[Breakpoint]
    B --> C[Pause]
    C --> D[Ispeziona variabili]
    D --> E[Step Over / Into / Out]
    E --> F[Individua la causa]
    F --> G[Correggi e riesegui]
```
...
Un progetto AI condiviso su GitHub può includere un file `requirements.txt`. Chi lo clona crea un ambiente virtuale e lancia `pip install -r requirements.txt`, ottenendo le dipendenze dichiarate dal progetto senza interferire con gli altri ambienti della macchina. L'ambiente risultante non è necessariamente **bit-per-bit identico**: per questo, nei progetti più rigorosi si considerano anche versione di Python, piattaforma e strumenti di lock delle dipendenze.
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 220" role="img" aria-label="Percorso del laboratorio Python">
  <g font-family="sans-serif">
    <rect x="30" y="70" width="190" height="90" rx="16" fill="none" stroke="currentColor" stroke-width="3"/>
    <rect x="270" y="70" width="190" height="90" rx="16" fill="none" stroke="currentColor" stroke-width="3"/>
    <rect x="510" y="70" width="190" height="90" rx="16" fill="none" stroke="currentColor" stroke-width="3"/>
    <rect x="750" y="70" width="220" height="90" rx="16" fill="none" stroke="currentColor" stroke-width="3"/>
    <text x="125" y="105" text-anchor="middle" font-size="22" font-weight="bold">1 · Setup</text>
    <text x="125" y="133" text-anchor="middle" font-size="17">venv + dipendenze</text>
    <text x="365" y="105" text-anchor="middle" font-size="22" font-weight="bold">2 · Dataset</text>
    <text x="365" y="133" text-anchor="middle" font-size="17">Pandas + DataFrame</text>
    <text x="605" y="105" text-anchor="middle" font-size="22" font-weight="bold">3 · Classe</text>
    <text x="605" y="133" text-anchor="middle" font-size="17">OOP + metodi</text>
    <text x="860" y="105" text-anchor="middle" font-size="22" font-weight="bold">4 · Verifica</text>
    <text x="860" y="133" text-anchor="middle" font-size="17">grafico + debugger</text>
    <path d="M220 115 H270 M460 115 H510 M700 115 H750" stroke="currentColor" stroke-width="3"/>
    <path d="M255 105 L270 115 L255 125 M495 105 L510 115 L495 125 M735 105 L750 115 L735 125" stroke="currentColor" stroke-width="3" fill="none"/>
  </g>
</svg>
...
> ⚠️ **Nota didattica:** nel progetto reale, dati scolastici o personali non vanno usati come dataset di prova se non è necessario. Per il laboratorio utilizza dati fittizi e non identificativi.
...
```mermaid
flowchart TD
    A[Nuovo progetto] --> B[Crea venv]
    B --> C[Installa dipendenze]
    C --> D[Verifica interprete]
    D --> E[Salva requirements.txt]
    E --> F[Codice + docstring]
    F --> G[Debug e test]
    G --> H[Versiona il progetto]
```
...
- ❌ **Installare librerie senza aver attivato l'ambiente virtuale.** Il prefisso `(venv)` nel terminale è un utile segnale che l'ambiente è attivo; per una verifica certa puoi controllare quale interprete Python sta usando il terminale.
...
- ❌ **Ignorare i messaggi di errore (traceback).** L'ultima parte indica il tipo di eccezione e il punto in cui è stata sollevata; conviene leggere il traceback nel suo insieme, risalendo alla prima riga del proprio codice coinvolta.
- ❌ **Non versionare il file `requirements.txt`** insieme al codice: rende molto più difficile ricostruire le dipendenze dichiarate del progetto su un altro computer.
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 250" role="img" aria-label="Tre fondamenta del modulo Python per AI">
  <circle cx="170" cy="125" r="82" fill="none" stroke="currentColor" stroke-width="3"/>
  <circle cx="450" cy="125" r="82" fill="none" stroke="currentColor" stroke-width="3"/>
  <circle cx="730" cy="125" r="82" fill="none" stroke="currentColor" stroke-width="3"/>
  <g font-family="sans-serif" text-anchor="middle">
    <text x="170" y="118" font-size="27" font-weight="bold">Ambiente</text>
    <text x="170" y="148" font-size="18">venv · pip</text>
    <text x="450" y="118" font-size="27" font-weight="bold">Dati</text>
    <text x="450" y="148" font-size="18">NumPy · Pandas</text>
    <text x="730" y="118" font-size="27" font-weight="bold">Codice</text>
    <text x="730" y="148" font-size="18">OOP · Debug</text>
  </g>
</svg>
...
| Termine | Definizione |
|---|---|
| **Ambiente virtuale (`venv`)** | Ambiente Python isolato, con le proprie librerie, separato dall'ambiente globale del sistema |
| **pip** | Strumento standard dell'ecosistema Python per installare e gestire pacchetti dal Python Package Index e da altre sorgenti configurate |
