...
C'è un principio che ogni AI Engineer impara presto, spesso a sue spese: **"garbage in, garbage out"**. Un modello di Machine Learning, per quanto sofisticato, non potrà mai produrre risultati migliori della qualità dei dati con cui viene addestrato. Una parte molto significativa del lavoro in Data Science è dedicata a **reperire, comprendere, pulire e preparare i dati**, prima ancora di costruire un modello. La percentuale esatta varia molto in base al progetto, alla qualità delle fonti e al contesto, quindi è più utile considerare questo principio operativo che affidarsi a una percentuale universale.
...
### 🧭 Mappa del flusso di Data Analysis

```mermaid
flowchart LR
    A[Reperimento dati] --> B[EDA]
    B --> C[Pulizia]
    C --> D[Preprocessing]
    D --> E[Feature Engineering]
    E --> F[Visualizzazione]
    F --> G[Dataset pronto per ML]
    B -. ipotesi .-> F
    F -. nuove domande .-> B
```

> **Idea chiave:** la Data Analysis è un processo iterativo. Una visualizzazione può far emergere un’anomalia che richiede di tornare all’EDA o alla fase di pulizia.
...
- **Repository pubblici**: Kaggle e UCI Machine Learning Repository sono esempi di fonti utili per esercitarsi su dataset reali e documentati. Prima di utilizzare un dataset, verifica sempre licenza, provenienza, descrizione delle colonne e modalità di raccolta.
- **API pubbliche**: molti enti e servizi (ad esempio ISTAT, Eurostat e alcuni servizi meteorologici) espongono dati tramite API interrogabili direttamente da Python. Verifica autenticazione, limiti di utilizzo, licenza e formato della risposta.
...
```mermaid
flowchart TD
    A[Dataset sorgente] --> B{Fonte e formato}
    B -->|CSV| C[pd.read_csv]
    B -->|Excel| D[pd.read_excel]
    B -->|API / URL| E[Caricamento da sorgente]
    C --> F[Controlli iniziali]
    D --> F
    E --> F
    F --> G[EDA]
```

> **Regola pratica:** prima di analizzare i valori, verifica anche provenienza, licenza, unità di misura, significato delle colonne e formato delle date.
...
Una buona EDA risponde almeno a tre domande: *cosa contiene* il dataset, *quanto è affidabile e coerente*, *quali pattern o anomalie posso aspettarmi* in base al dominio del problema. L’EDA non serve solo a descrivere i dati: serve anche a formulare ipotesi da verificare.

```mermaid
flowchart LR
    A[Struttura] --> D[EDA]
    B[Qualità] --> D
    C[Distribuzioni e categorie] --> D
    D --> E[Ipotesi]
    E --> F[Visualizzazioni / verifiche]
```
...
Non esiste una regola unica: eliminare righe è semplice ma può far perdere informazione; l’imputazione preserva le righe ma introduce un’assunzione. Prima della scelta bisogna capire **perché** i valori mancano e se il meccanismo di assenza può essere legato al fenomeno osservato. In un progetto di Machine Learning, i parametri usati per imputare i dati devono essere stimati sul training set e poi applicati agli altri set, per evitare data leakage.
...
#### Infografica — qualità dei dati

<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 180" role="img" aria-label="Strategia per la qualità dei dati">
  <style>text{font-family:system-ui,sans-serif;fill:currentColor}.box{fill:none;stroke:currentColor;stroke-width:2}.arrow{fill:none;stroke:currentColor;stroke-width:2}</style>
  <rect class="box" x="20" y="45" width="250" height="90" rx="14"/><rect class="box" x="325" y="45" width="250" height="90" rx="14"/><rect class="box" x="630" y="45" width="250" height="90" rx="14"/>
  <path class="arrow" d="M270 90h55m250 0h55"/>
  <text x="145" y="78" text-anchor="middle" font-weight="700">1 · Identifica</text><text x="145" y="106" text-anchor="middle">missing · duplicati · outlier</text>
  <text x="450" y="78" text-anchor="middle" font-weight="700">2 · Comprendi</text><text x="450" y="106" text-anchor="middle">causa · dominio · impatto</text>
  <text x="755" y="78" text-anchor="middle" font-weight="700">3 · Decidi</text><text x="755" y="106" text-anchor="middle">elimina · imputa · correggi</text>
</svg>

> L’IQR è uno strumento di individuazione, non una prova automatica che un valore sia un errore: la decisione finale richiede contesto e conoscenza del dominio.

### 4.4 — Preprocessing: normalizzazione, standardizzazione, encoding

Molti algoritmi di Machine Learning (che affronterai nel Modulo 6) beneficiano di feature numeriche su scale comparabili; per altri algoritmi il ridimensionamento può essere meno importante. Le variabili categoriche, inoltre, devono essere rappresentate in un formato compatibile con il modello.
...
# Encoding ordinale: usare numeri solo quando esiste un ordine reale
ordine = {"Basso": 0, "Medio": 1, "Alto": 2}
df["livello_cod"] = df["livello"].map(ordine)
```

> 💡 Per categorie **nominali** senza ordine naturale (es. "Matematica", "Storia", "Arte"), One-Hot Encoding evita di introdurre un ordine artificiale. Per categorie **ordinali** (es. "Basso", "Medio", "Alto") si può usare una codifica numerica esplicitamente definita. Evita codici numerici arbitrari per variabili nominali, perché il modello potrebbe interpretarli come valori ordinati.

```mermaid
flowchart TD
    A[Variabile categorica] --> B{Esiste un ordine reale?}
    B -->|No| C[One-Hot Encoding]
    B -->|Sì| D[Encoding ordinale]
    C --> E[Feature numeriche]
    D --> E
    E --> F[Modello ML]
```

> **Attenzione al leakage:** scaler, imputatori ed encoder che apprendono parametri dai dati dovranno essere adattati sul training set e poi applicati a validation/test. Il Modulo 6 mostrerà il flusso completo.
...
Una buona feature engineering può migliorare sensibilmente l’informazione disponibile al modello, ma il suo effetto dipende dal dataset e dall’algoritmo: non esiste una regola secondo cui debba sempre contare più della scelta del modello. Nel Modulo 6 vedrai come valutarne l’effetto senza introdurre leakage.

#### Infografica — dalla materia prima alla feature

<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 190" role="img" aria-label="Feature engineering: trasformare dati esistenti in feature utili">
  <style>text{font-family:system-ui,sans-serif;fill:currentColor}.box{fill:none;stroke:currentColor;stroke-width:2}.arrow{fill:none;stroke:currentColor;stroke-width:2}</style>
  <rect class="box" x="25" y="50" width="190" height="90" rx="14"/><rect class="box" x="355" y="50" width="190" height="90" rx="14"/><rect class="box" x="685" y="50" width="190" height="90" rx="14"/>
  <path class="arrow" d="M215 95h140m190 0h140"/>
  <text x="120" y="83" text-anchor="middle" font-weight="700">Dati esistenti</text><text x="120" y="111" text-anchor="middle">date · voti · categorie</text>
  <text x="450" y="83" text-anchor="middle" font-weight="700">Trasformazione</text><text x="450" y="111" text-anchor="middle">aggregare · estrarre · combinare</text>
  <text x="780" y="83" text-anchor="middle" font-weight="700">Nuove feature</text><text x="780" y="111" text-anchor="middle">informazione più utile</text>
</svg>
...
La heatmap di correlazione è utile in fase esplorativa: mostra associazioni lineari tra variabili numeriche e può evidenziare possibili ridondanze o relazioni da approfondire. **Non dimostra causalità**, non cattura ogni tipo di relazione e una correlazione con il target non implica automaticamente che la feature sia utile al modello.

```mermaid
flowchart LR
    A[Domanda analitica] --> B{Tipo di informazione}
    B -->|Distribuzione| C[Istogramma]
    B -->|Gruppi + valori estremi| D[Boxplot]
    B -->|Relazione tra 2 numeriche| E[Scatterplot]
    B -->|Associazioni multiple| F[Heatmap]
    C --> G[Interpretazione]
    D --> G
    E --> G
    F --> G
...
- ✅ Applica normalizzazione/standardizzazione e imputazione **senza far apprendere i parametri all’intero dataset**: nel flusso ML, i parametri vanno appresi dal training set e poi applicati a validation/test, per evitare data leakage.
...
```mermaid
flowchart TD
    A[Dataset originale] --> B[Train / Validation / Test]
    B --> C[Fit: imputer + scaler + encoder]
    C --> D[Transform]
    D --> E[Modello]
    B -. test isolato .-> F[Valutazione finale]
```

> Il principio fondamentale è: **prima si separano i dati, poi si apprendono i parametri delle trasformazioni dal training set**.
...
- ❌ **Usare automaticamente la media per l'imputazione**: su distribuzioni asimmetriche o con outlier la mediana può essere più robusta, ma la scelta va motivata rispetto al dominio e al meccanismo dei missing.
- ❌ **Applicare One-Hot Encoding senza considerare l’elevata cardinalità**: molte categorie possono produrre moltissime colonne; la strategia va scelta in base al dominio, alla frequenza delle categorie e al modello.
- ❌ **Confondere normalizzazione e standardizzazione**: producono scale diverse e sono adatte a contesti diversi.
- ❌ **Interpretare correlazione come causalità**: un’associazione osservata nella heatmap non dimostra che una variabile "causi" l’altra; possono intervenire fattori esterni o coincidenze.
...
Nel prossimo modulo (Machine Learning) userai il dataset che hai imparato a preparare qui come punto di partenza per addestrare, validare e valutare il tuo primo modello predittivo.

<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 190" role="img" aria-label="Tre pilastri della preparazione dei dati">
  <style>text{font-family:system-ui,sans-serif;fill:currentColor}.c{fill:none;stroke:currentColor;stroke-width:2}</style>
  <circle class="c" cx="170" cy="95" r="68"/><circle class="c" cx="450" cy="95" r="68"/><circle class="c" cx="730" cy="95" r="68"/>
  <text x="170" y="90" text-anchor="middle" font-weight="700">Qualità</text><text x="170" y="114" text-anchor="middle">dati affidabili</text>
  <text x="450" y="90" text-anchor="middle" font-weight="700">Trasformazione</text><text x="450" y="114" text-anchor="middle">feature utili</text>
  <text x="730" y="90" text-anchor="middle" font-weight="700">Interpretazione</text><text x="730" y="114" text-anchor="middle">evidenze, non causalità</text>
</svg>
...
| **Correlazione** | Misura statistica dell’associazione, tipicamente lineare, tra due variabili numeriche; non dimostra causalità |
| **Data leakage** | Uso involontario, nel processo di addestramento o preparazione, di informazioni che non dovrebbero essere disponibili al modello in quel momento, producendo una valutazione troppo ottimistica |
