...
```mermaid
flowchart TD
    A["🧱 Fondamenti<br/>HTML, struttura, primo file"]:::s1 --> B["🔍 Anatomia dei tag<br/>commenti e nidificazione"]:::s2
    B --> C["✍️ Titoli e testo<br/>h1-h6, paragrafi, formattazione"]:::s3
...
*📊 Il percorso del Modulo 2: dalle basi alla formattazione del testo.*
...
```mermaid
flowchart LR
    H["🦴 HTML<br/>Struttura<br/>cosa c'è nella pagina"]:::cHtml --> P(["🌐 Pagina Web"]):::cPage
    C["🎨 CSS<br/>Aspetto<br/>come appare"]:::cCss --> P
    J["⚡ JavaScript<br/>Comportamento<br/>cosa fa"]:::cJs --> P
    classDef cHtml fill:#fde8d0,stroke:#e34c26,color:#222
    classDef cCss fill:#d6e6fb,stroke:#264de4,color:#222
    classDef cJs fill:#fff6c2,stroke:#c9a800,color:#222
    classDef cPage fill:#e3f4dc,stroke:#3d8b37,color:#222
```

*📊 I tre linguaggi del Web: HTML descrive, CSS abbellisce, JavaScript anima.*

> 💡 **Approfondimento** — HTML non è un linguaggio di *programmazione*, ma di *markup* ("marcatura"): non contiene istruzioni, cicli o condizioni, ma soltanto etichette che descrivono i contenuti. Per questo è il punto di partenza ideale: potrai vedere risultati concreti nel browser fin dalle prime righe.
...
Per convenzione i nomi dei tag si scrivono sempre in **minuscolo** (`<p>`, non `<P>`): il browser non farebbe distinzione, ma la convenzione rende il codice più uniforme e leggibile.
...
```mermaid
flowchart TD
    subgraph EL["Elemento HTML"]
        A["#lt;p#gt;<br/>Tag di apertura"]:::tag --> B["Testo del paragrafo<br/>Contenuto"]:::content --> C["#lt;/p#gt;<br/>Tag di chiusura"]:::tag
    end
    classDef tag fill:#fde8d0,stroke:#e34c26,color:#222
    classDef content fill:#eef2f7,stroke:#64748b,color:#222
```

*📊 Il tag è il singolo marcatore; l'elemento è l'insieme completo apertura + contenuto + chiusura.*
...
```mermaid
flowchart TD
    E["Elemento #lt;a#gt;"]:::tag
    E --> A["Attributo"]:::attr
    E --> C["Contenuto<br/>Google"]:::content
    A --> N["Nome<br/>href"]:::attr
    A --> V["Valore<br/>#quot;https://www.google.it#quot;"]:::attr
    classDef tag fill:#fde8d0,stroke:#e34c26,color:#222
    classDef attr fill:#d6e6fb,stroke:#264de4,color:#222
    classDef content fill:#eef2f7,stroke:#64748b,color:#222
```

*📊 Anatomia di un attributo: una coppia nome="valore" scritta nel tag di apertura.*
...
> ⚠️ **Attenzione** — Scrivi sempre i valori tra virgolette doppie e separa i vari attributi con uno spazio: dimenticare le virgolette o lo spazio (`href="..."target="_blank"`) è un errore molto frequente e può impedire al browser di interpretare correttamente il tag.
...
Ecco come questi elementi si dispongono, tipicamente, all'interno di una pagina:

```mermaid
flowchart TD
    subgraph PAG["📄 Pagina web"]
        H["header<br/>intestazione"]:::sem
        N["nav<br/>navigazione"]:::sem
        S["section / article<br/>contenuto"]:::sem
        A["aside<br/>secondario"]:::sem
        F["footer<br/>piè di pagina"]:::sem
    end
    H ~~~ N
    N ~~~ S
    N ~~~ A
    S ~~~ F
    A ~~~ F
    classDef sem fill:#e3f4dc,stroke:#3d8b37,color:#222
```

*📊 Disposizione tipica degli elementi semantici HTML5 in una pagina.*
...
> 💡 **Suggerimento** — Quando avrai preso confidenza, in Visual Studio Code potrai generare lo scheletro automaticamente: in un file `.html` digita `!` e premi `Tab` (o `Invio`). Ricordati però di sostituire `lang="en"` con `lang="it"`.
...
Il browser legge il documento dall'alto verso il basso e lo organizza come un **albero**: ogni elemento contenuto in un altro ne diventa il "figlio". Ecco l'albero della pagina appena vista (`<!DOCTYPE html>` lo precede e dichiara soltanto la versione del linguaggio):

```mermaid
flowchart LR
    R["html<br/>lang=#quot;it#quot;"]:::root
    R --> HD["head"]:::head
    R --> BD["body"]:::body
    HD --> M1["meta<br/>charset"]:::leaf
    HD --> M2["meta<br/>viewport"]:::leaf
    HD --> T["title"]:::leaf
    BD --> X["Ciao Mondo!"]:::leaf
    classDef root fill:#fde8d0,stroke:#e34c26,color:#222
    classDef head fill:#d6e6fb,stroke:#264de4,color:#222
    classDef body fill:#e3f4dc,stroke:#3d8b37,color:#222
    classDef leaf fill:#eef2f7,stroke:#64748b,color:#222
```

*📊 L'albero del documento: `<head>` contiene le informazioni sulla pagina, `<body>` il contenuto visibile.*
...
> 💡 **Approfondimento** — Il `<title>` non compare nel corpo della pagina, ma è visibile nella scheda del browser, tra i preferiti e come titolo del risultato nei motori di ricerca: scegline sempre uno descrittivo.
...
> ⚠️ **Non vedi nulla, o Live Server non compare?** — Controlla che l'estensione sia installata (scheda *Estensioni* di Visual Studio Code, `Ctrl+Shift+X`, cerca "Live Server") e che il file sia davvero un `.html`: su alcuni sistemi le estensioni dei file sono nascoste e `index.html` potrebbe in realtà chiamarsi `index.html.txt`.
...
Abituarti fin da subito a questa organizzazione ti farà risparmiare tempo prezioso quando lavorerai a progetti reali di dimensioni maggiori. Il punto di partenza è sempre `index.html`, da cui richiamerai le risorse conservate nelle altre cartelle:

```mermaid
flowchart LR
    P["📄 index.html<br/>la tua pagina"]:::page
    P -- "stile" --> C["📁 css/<br/>style.css"]:::folder
    P -- "immagini" --> I["📁 images/"]:::folder
    P -- "script" --> J["📁 js/"]:::folder
    P -- "font, icone" --> A["📁 assets/"]:::folder
    classDef page fill:#fde8d0,stroke:#e34c26,color:#222
    classDef folder fill:#fff6c2,stroke:#c9a800,color:#222
```

*📊 Dalla pagina index.html si richiamano le risorse conservate nelle cartelle dedicate (vedremo come nei prossimi moduli).*
...
✔ Scrivere nomi di file e cartelle in minuscolo, senza spazi né lettere accentate (es. `chi-siamo.html`).
...
❌ Usare spazi, maiuscole o accenti nei nomi dei file (es. `Chi Siamo.html`).
...
flowchart TD
    T["Tag HTML"]:::root
    T --> W["Con contenuto<br/>apertura + chiusura"]:::ok
    T --> V["Void elements<br/>solo apertura"]:::void
    W --> W1["#lt;p#gt; #lt;h1#gt;<br/>#lt;strong#gt; #lt;em#gt;"]:::leaf
    V --> V1["#lt;br#gt; #lt;hr#gt; #lt;img#gt;<br/>#lt;meta#gt; #lt;link#gt; #lt;input#gt;"]:::leaf
    classDef root fill:#eef2f7,stroke:#64748b,color:#222
    classDef ok fill:#e3f4dc,stroke:#3d8b37,color:#222
    classDef void fill:#fff6c2,stroke:#c9a800,color:#222
    classDef leaf fill:#ffffff,stroke:#94a3b8,color:#222
```

*📊 Due famiglie di tag: quelli che racchiudono un contenuto e quelli che non hanno chiusura.*
...
> ✅ **Consiglio pratico** — Per indentare correttamente l'intero file premi `Shift+Alt+F` (su macOS `Shift+Option+F`): VS Code formatta il documento per te. Puoi scegliere 2 oppure 4 spazi per ogni livello (in questo modulo troverai entrambe le convenzioni), l'importante è mantenere sempre la stessa scelta all'interno dello stesso progetto.
...
> ⚠️ **Attenzione** — I commenti non si vedono nella pagina, ma restano leggibili da chiunque ne consulti il codice sorgente (tasto destro → *Visualizza sorgente pagina*): non inserirvi mai password o informazioni riservate.
...
Nel confronto che segue, le due sequenze mostrano l'ordine con cui i tag vengono aperti e chiusi:

```mermaid
%%{init: {"flowchart": {"rankSpacing": 25, "nodeSpacing": 40}}}%%
flowchart TD
    subgraph OK["✅ Corretto"]
        O1["#lt;p#gt;"]:::ok --> O2["#lt;strong#gt;"]:::ok --> O3["Testo"]:::txt --> O4["#lt;/strong#gt;"]:::ok --> O5["#lt;/p#gt;"]:::ok
    end
    subgraph KO["❌ Errato"]
        K1["#lt;p#gt;"]:::ko --> K2["#lt;strong#gt;"]:::ko --> K3["Testo"]:::txt --> K4["#lt;/p#gt;"]:::ko --> K5["#lt;/strong#gt;"]:::ko
    end
    classDef ok fill:#d4edda,stroke:#2e7d32,color:#1b3d1f
    classDef ko fill:#f8d7da,stroke:#c62828,color:#5c1a1f
    classDef txt fill:#eef2f7,stroke:#64748b,color:#222
```

*📊 L'ultimo tag aperto deve essere il primo a chiudersi: a destra `<p>` si chiude mentre `<strong>` è ancora aperto.*
...
```mermaid
flowchart LR
    H1["h1<br/>Corso HTML"]:::h1 --> H2A["h2<br/>Introduzione"]:::h2
    H1 --> H2B["h2<br/>Strumenti"]:::h2
    H2A --> H3A["h3<br/>Cos'è HTML"]:::h3
    H2A --> H3B["h3<br/>Storia"]:::h3
    H2B --> H3C["h3<br/>Editor"]:::h3
    classDef h1 fill:#fde8d0,stroke:#e34c26,color:#222
    classDef h2 fill:#d6e6fb,stroke:#264de4,color:#222
    classDef h3 fill:#e3f4dc,stroke:#3d8b37,color:#222
```

*📊 La gerarchia dei titoli funziona come l'indice di un libro: un solo h1, poi i livelli in ordine.*
...
> 💡 **Approfondimento** — I titoli funzionano come l'indice di un libro: chi usa uno screen reader può scorrere la pagina passando da un titolo all'altro per orientarsi rapidamente. Una gerarchia ordinata rende quindi la pagina più accessibile, oltre che più chiara per i motori di ricerca.
...
> ⚠️ **Attenzione** — Nel codice puoi andare a capo e inserire quanti spazi vuoti vuoi per mantenerlo leggibile, ma il browser li riduce sempre a un solo spazio. Per separare due blocchi di testo serve un nuovo `<p>`; per andare a capo all'interno dello stesso paragrafo serve il tag `<br>`, che vedremo subito.
...
È il tag giusto quando l'a capo fa parte del contenuto stesso, ad esempio in un indirizzo postale o nei versi di una poesia.
...
> 💡 **Approfondimento** — Esistono anche i tag `<b>` e `<i>`, che producono lo stesso effetto visivo (grassetto e corsivo) ma senza attribuire al testo importanza o enfasi. Per i contenuti che contano davvero, preferisci `<strong>` ed `<em>`.
...
```html
<p>Il prezzo è <del>50 €</del> <ins>39 €</ins>: <mark>offerta valida fino a domenica</mark>.</p>

<p>L'acqua si scrive H<sub>2</sub>O, mentre l'area di un quadrato è l<sup>2</sup>.</p>

<p><small>Prezzi IVA inclusa.</small></p>
```
...
✔ Scrivere il testo dentro i tag appropriati (`<p>`, titoli) e non affidarsi a spazi o righe vuote nel codice per l'impaginazione.
...
❌ Contare su spazi multipli e righe vuote nel codice per impaginare il testo.
...
Metti alla prova quanto imparato in questo modulo con i seguenti esercizi pratici. Ti consigliamo di svolgerli in ordine, salvando ogni esercizio in un file `.html` separato dentro la cartella `web-programming` creata al paragrafo 2.8 e organizzata secondo la struttura del paragrafo 2.9.
...
6. **Caccia all'errore (bonus)** — Copia in un nuovo file il codice qui sotto, individua e correggi tutti gli errori (ce ne sono almeno cinque), poi verifica il risultato con Live Server.
...
<body>
    <h1>Benvenuti</h1>
    <h1>Il mio sito</h1>
    <p>Questo testo è <strong>molto importante</p></strong>
    <br><br><br>
</body>
</html>
```
