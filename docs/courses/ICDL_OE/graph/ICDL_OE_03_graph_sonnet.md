...
```mermaid
xychart-beta
    title "Segnale analogico (continuo) vs digitale (a livelli discreti)"
    x-axis ["t0","t1","t2","t3","t4","t5","t6","t7"]
    y-axis "Ampiezza" -1 --> 1
    line [0, 0.7, 1, 0.7, 0, -0.7, -1, -0.7]
    bar [0, 1, 1, 1, 0, 0, 0, 0]
```

*La linea rappresenta l'onda analogica continua; le barre rappresentano il segnale digitale, che può assumere solo valori discreti (qui, 0 o 1).*
...
```mermaid
flowchart LR
    A["📈 Segnale analogico<br/>(continuo)"] --> B["🕒 Campionamento<br/>(misura a intervalli regolari)"]
    B --> C["🔢 Quantizzazione<br/>(approssima al livello più vicino)"]
    C --> D["💾 Segnale digitale<br/>(sequenza di bit)"]
...
```mermaid
flowchart LR
    P["PAN<br/>pochi metri"] --> L["LAN<br/>edificio/scuola"] --> M["MAN<br/>città"] --> W["WAN<br/>stato/mondo"]
    style P fill:#dbeafe,stroke:#1e3a8a
    style L fill:#bfdbfe,stroke:#1e3a8a
    style M fill:#93c5fd,stroke:#1e3a8a
    style W fill:#60a5fa,stroke:#1e3a8a,color:#ffffff
```
...
flowchart TD
    ...
    PC3((PC3)) --- SW
    PC4((PC4)) --- SW
    style SW fill:#fed7aa,stroke:#7c2d12
```
...
```mermaid
flowchart TD
    I["🌐 Internet"] --> R["Router<br/>(+ Access Point Wi-Fi)"]
    R -. Wi-Fi .-> W["📱 💻 dispositivi wireless"]
    R --> S["Switch interno"]
    S --> PC["🖥️ PC fisso (Ethernet)"]
    S --> PR["🖨️ Stampante di rete (Ethernet)"]
    style I fill:#f3f4f6,stroke:#374151
    style R fill:#dbeafe,stroke:#1e3a8a
...
```mermaid
xychart-beta
    title "Collegamenti necessari al crescere della rete"
    x-axis ["5 nodi", "10 nodi", "20 nodi"]
    y-axis "Numero di collegamenti" 0 --> 200
    line [5, 10, 20]
    bar [10, 45, 190]
```

*Linea = Bus/Stella (crescita lineare) · Barre = Mesh (crescita quadratica).*
