      ...
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
        ...
        <nav aria-label="Navigazione principale">
          <ul className="flex items-center gap-8 text-sm">
            ...
            {/* 🎯 DASHBOARD: Sostituisce il vecchio CMS Admin statico con la rotta V2 */}
            {user?.role === "admin" && (
              <li>
                <Link
        ...
        <div className="flex items-center gap-4">
          {isLoading ? (
            ...
            <div className="flex items-center gap-3">
              ...
              <Link
                href="/profile"
                className="relative flex h-9 w-9 shrink-0 overflow-hidden rounded-full border border-gray-200 shadow-sm hover:opacity-85 transition-opacity duration-200"
              >
                {user.avatarUrl ? (
                  <img
                    src={user.avatarUrl}
              ...
              <button
                onClick={logout}
                className="rounded-md border border-gray-300 bg-white px-3 py-1.5 text-xs font-medium text-gray-700 shadow-sm hover:bg-gray-50 transition-all ml-1"
              >
                Logout
              </button>
            </div>
