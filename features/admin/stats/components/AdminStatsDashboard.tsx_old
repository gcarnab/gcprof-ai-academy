...
export default function AdminStatsDashboard({ stats }: Props) {
  ...
  // Se nel servizio PostgreSQL/Supabase hai estratto le classi aggregate (es. '5A-INF' o 'Nessuna'),
  // le mappiamo direttamente per il grafico, altrimenti impostiamo un fallback sicuro.
  const refinedStudentEngagement = (stats.charts.studentEngagement || []).map((student: any) => ({
    name: student.name,
    hours: student.hours,
    classes: student.classes || student.class_name || undefined // 👈 Aggancia automaticamente la colonna del DB
  }));
  ...
  return (
    <div className="space-y-10 p-6">
      ...
      <div className="space-y-4">
        ...
        {/* 🎯 RIGRAFO AGGIORNATO: Tempo di Attività Studenti con supporto Classi */}
        <div className="pt-2">
          <HorizontalBarChartCard
            title="Tempo di Attività Studenti"
            subtitle="Classifica del tempo totale cumulato dagli studenti all'interno dei corsi"
            data={refinedStudentEngagement} // 👈 Passiamo i dati raffinati con il supporto classi
          />
        </div>
      </div>
