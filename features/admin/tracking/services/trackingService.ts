// gcprof-ai-academy/features/admin/tracking/services/trackingService.ts
...
export class TrackingService {
  ...
  static async createSession(profileId: string) {
    ...
    try {
      ...
      const UNKNOWN_VALUE =
        process.env.TRACKING_UNKNOWN_VALUE ?? "unknown";
      ...
      const userAgent =
        headerStore.get("user-agent") ?? UNKNOWN_VALUE;
      ...
      logger.info(
        `Tracking session creata per utente ${profileId}`,
      );
    } catch (error) {
      logger.error(
        `TrackingService.createSession exception: ${String(error)}`,
      );
    }
  }
...
// 🛑 KILL SWITCH: se disabilitato, interrompe subito l'esecuzione
    ...
    try {
      ...
      if (!session) {
        logger.warn(
          `Nessuna sessione aperta trovata per utente ${profileId}`,
        );
      ...
      logger.error(
        `TrackingService.closeSession exception: ${String(error)}`,
      );
    }
  }

  /**
   * Page-view tracking disabilitato nel LIVELLO 1.
   *
   * Manteniamo intenzionalmente il metodo per non modificare
   * i caller esistenti e non introdurre regressioni.
   *
   * Il metodo non effettua più alcuna scrittura su:
...
*/
  ...
  static async getSessions() {
    try {
      ...
      if (error) {
        logger.error(
          `TrackingService.getSessions error: ${error.message}`,
        );
      ...
      logger.error(
        `TrackingService.getSessions exception: ${String(error)}`,
      );
