...
export default function RequestsTab() {
  ...
  // Stati della vista
  const [requestView, setRequestView] = useState<"pending" | "active" | "revoked">("pending");
      ...
      try {
        ...
        if (result.success) {
          setPendingRequests((prev) =>
            prev.filter((req) => !(req.profileId === profileId && req.courseId === courseId))
          );
  ...
  const handleRevokeEnrollment = (profileId: string, courseId: string) => {
    if (!confirm("Sei sicuro di voler revocare l'accesso a questo studente? L'operazione è immediata.")) return;
  ...
  const handleReactivateEnrollment = (profileId: string, courseId: string) => {
    startReactivateTransition(async () => {
      try {
        const result = await reactivateExternalEnrollmentAction(profileId, courseId);
        if (result.success) {
                    ...
                    >
                      ...
                      <td className="p-4 text-right">
                        <Button
                          ...
                          onClick={() => handleApproveEnrollment(req.profileId, req.courseId)}
                          disabled={isPendingApproval}
                        >
                          {isPendingApproval ? "Abilitazione..." : "✅ Approva Accesso"}
                        </Button>
                      </td>
                    </tr>
                  ))}
                    ...
                    >
                      ...
                      <td className="p-4 text-right">
                        <Button
                          ...
                          onClick={() => handleRevokeEnrollment(req.profileId, req.courseId)}
                          disabled={isPendingRevoke}
                        >
                          {isPendingRevoke ? "Revoca in corso..." : "🚫 Revoca Accesso"}
                        </Button>
                      </td>
                    </tr>
                  ))}
                    ...
                    >
                      ...
                      <td className="p-4 text-right">
                        <Button
                          ...
                          onClick={() => handleReactivateEnrollment(req.profileId, req.courseId)}
                          disabled={isPendingReactivate}
                        >
                          {isPendingReactivate ? "Ripristino..." : "🔄 Ripristina Accesso"}
                        </Button>
                      </td>
                    </tr>
                  ))}
