...
export default async function StudentDashboardPage({ searchParams }: PageProps) {
  ...
  // Dati Gamification Multi-Corso
  const gamificationOverview = gamificationRes.success && gamificationRes.data ? gamificationRes.data : null;
  const totalXp = gamificationOverview?.global.total_xp ?? profileData?.total_xp ?? 0;
  const currentLevel = gamificationOverview?.global.global_level ?? profileData?.current_level ?? 1;
  const courseStatsList = gamificationOverview?.courses || [];

  // Mappa comoda per accedere velocemente alle metriche del corso
  const courseStatsMap = new Map(courseStatsList.map((c) => [c.course_id, c]));
  ...
  // 🧮 LOGICA PROGRESSO % + GAMIFICATION PER CORSO
  const coursesWithRealMetrics = allowedCourses.map((allowedCourse: any) => {
    ...
    const courseGamification = courseStatsMap.get(allowedCourse.course_id);
    ...
    return {
      ...
      percentage,
      courseXp: courseGamification?.course_xp || 0,
      courseLevel: courseGamification?.course_level || 1,
      courseBadges: courseGamification?.badges || []
    };
  });

  // 📝 MAPPATURA DEI QUIZ ATTEMPTS
      ...
      <div className="p-8 max-w-7xl mx-auto space-y-8">
        ...
        <div className="bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-800 rounded-2xl p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-xl transition-colors">
          <div className="flex items-center gap-4">
            <div className="h-12 w-12 bg-gradient-to-tr from-blue-600 to-indigo-600 rounded-full flex items-center justify-center font-black text-lg text-white shrink-0">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-6 border-t md:border-t-0 md:border-l border-slate-200 dark:border-slate-800 pt-4 md:pt-0 md:pl-8">
            ...
            {/* WIDGET COMPATTO GAMIFICATION GLOBALE */}
            <div className="border-t sm:border-t-0 sm:border-l border-slate-200 dark:border-slate-800 pt-3 sm:pt-0 sm:pl-6">
              <GamificationBar totalXp={totalXp} currentLevel={currentLevel} compact />
            </div>
          </div>
        </div>
                  ...
                  >
                    <div className="p-5 flex-1 space-y-3">
                      <div className="flex justify-between items-start gap-2">
                        ...
                        {/* WIDGET XP PER SINGOLO CORSO */}
                        <span className="text-[10px] font-extrabold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/50 px-2 py-0.5 rounded-full">
                          Lvl {course.courseLevel} • {course.courseXp} XP
                        </span>
                      </div>
