...
export default function LessonPage() {
  ...
  console.log("=== [CHECKPOINT 1] Componente avviato. Params correnti:", params);
  ...
  useEffect(() => {
    async function loadLessonData() {
      ...
      console.log("=== [CHECKPOINT 2] Avvio fetch dati per:", { slug, moduleId, lessonId });
      
      try {
        const liveCourses = await getLiveCourses();
        // 🔴 CHECKPOINT 3: I corsi live sono arrivati?
        console.log("=== [CHECKPOINT 3] Corsi scaricati dal service. Totale corsi:", liveCourses?.length);
        ...
        if (course && module && lesson) {
          console.log("=== [CHECKPOINT 4-OK] Lezione trovata con successo:", lesson);
          setData({ course, module, lesson });
        } else {
          console.warn("=== [CHECKPOINT 4-FAIL] Impossibile trovare la tripletta nei dati!");
          setData(null);
        }
      } catch (err) {
    ...
    if (slug && moduleId && lessonId) {
      ...
      console.log("=== [CHECKPOINT PARAMETRI MANCANTI]:", { slug, moduleId, lessonId });
    }
  }, [slug, moduleId, lessonId]);

  // 🔴 CHECKPOINT 5: Stato dello switch di rendering
  console.log("=== [CHECKPOINT 5] Stato render attuale:", { isLoading, haDati: !!data });
  ...
  if (!data) {
    ...
    console.log("=== [CHECKPOINT 6] Dati assenti. Innesco notFound() di Next.js");
    notFound();
    return null;
  }
  ...
  const normalizedType = (lesson.content_type || lesson.contentType || "").toLowerCase();
  const normalizedUrl = lesson.external_url || lesson.externalUrl || lesson.video_url || lesson.youtubeUrl || lesson.googleDriveUrl || "";
  ...
  console.log("=== [CHECKPOINT 7] Dati pronti per il Player:", formattedContents);
      ...
      <div className="flex min-h-screen flex-col bg-gray-50">
        ...
        <main className="mx-auto w-full max-w-5xl flex-1 px-6 py-10">
          ...
          <nav className="text-sm text-gray-500 flex flex-wrap items-center gap-2 select-none">
            ...
            <span className="text-gray-400">/</span>
            <span className="text-gray-700 font-medium max-w-[250px] truncate" title={module.title}>
              {module.title}
            </span>
          </nav>
